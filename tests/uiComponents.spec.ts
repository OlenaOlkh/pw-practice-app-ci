...
//  test.describe.configure({retries: 2})
  ...
  test('Input fields', async ({ page }/*, tesInfo*/) => {
    /*if(testInfo.retry){
      //do something
    }*/
    ...
    //locator assertion
    expect(usingTheGridEmailInput).toHaveValue('test1@test.com')
  });

  test.only('radio buttons', async ({ page }) => {
    const usingTheGridForm = page.locator('nb-card', { hasText: "Using the Grid" })
    await usingTheGridForm.getByRole('radio', {name: "Option 1"}).check({ force: true })
    await expect(usingTheGridForm).toHaveScreenshot({maxDiffPixels:109})
...
//28
  ...
  test('Datepicker', async ({ page }) => {
    ...
    let calendarMonthsAndYear = await page.locator('nb-calendar-view-mode').textContent()
    const expectedMonthAndYear = ` ${expectedMonthLong} ${expectedYear} `
    while(! calendarMonthsAndYear.includes(expectedMonthAndYear)){
      await page.locator('nb-calendar-pageable-navigation [data-name="chevron-right"]').click()
      calendarMonthsAndYear = await page.locator('nb-calendar-view-mode').textContent()
    }
  ...
  await tempGauge.evaluate( node => {
    node.setAttribute('cx', '258.238')
    node.setAttribute('cy', '196.396')
    })
